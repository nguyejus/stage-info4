    ...
    if (not ((Config.get_correl_mode v1 patt) = Ast_config.Nocorrel)) then
      try
		...
		begin
		  ...
		  let _ = if strict then (let logmsg=Printf.sprintf "*** INFO *** Strict correlation used for the pattern %s" patt in Bolt.Logger.log logmsg Bolt.Level.INFO "") in
	  ...
	  Bolt.Logger.log logmsg4 Bolt.Level.ERROR ""
