...
let get_diff_job v prefix difffile cmd =
  ...
  with
      Unix.WEXITED 0 -> ()
    | Unix.WEXITED 1 -> ()
    | Unix.WEXITED i -> (let logmsg=Printf.sprintf "*** FAILURE *** Code: %d %s" i cmd in Bolt.Logger.log logmsg Bolt.Level.ERROR "")
    | _ -> (let logmsg=Printf.sprintf "*** FAILURE *** %s" cmd in Bolt.Logger.log logmsg Bolt.Level.ERROR "")
...
let gen_cmd_basic v prefix pair difffile =
  ...
  Parmap.parfold (fun file_pair (outlist, cmdlist) ->
    ...
    if not (Sys.file_exists outfile) then
      (let logmsg=Printf.sprintf "Checking (%d) %s - Keep" (Unix.getpid()) outfile in Bolt.Logger.log logmsg Bolt.Level.TRACE "";
       (outfile::outlist,(outfile, cmd)::cmdlist))
    else
      (let logmsg=Printf.sprintf "Checking (%d) %s - Skip" (Unix.getpid()) outfile in Bolt.Logger.log logmsg Bolt.Level.TRACE "";
  ...
  match difffile with
	...
	List.flatten (
	  Parmap.parmap (fun x ->
	    ...
	    with e -> (let logmsg=Printf.sprintf "Error parsing %s" x in Bolt.Logger.log logmsg Bolt.Level.ERROR ""); raise e
	  ) (Parmap.L outfiles)
	)
    | Gumtree _ -> 
      List.flatten (
...
let show_diff verbose vlist ast =
  ...
  List.iter (fun ((ver, file), difftype) ->
    ...
    Bolt.Logger.log logmsg Bolt.Level.TRACE "");
    match difftype with
	Ast_diff.GNUDiff hunks -> (let logmsg=Printf.sprintf "%s" (show_gnudiff hunks) in Bolt.Logger.log logmsg Bolt.Level.TRACE "")
