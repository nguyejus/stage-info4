...
let clean_file exist editfile =
  if Sys.file_exists editfile then
    begin
      if exist then let msg=Printf.sprintf "*** INFO *** %s is thus removed." editfile in Bolt.Logger.log msg Bolt.Level.INFO "";
      Sys.remove editfile
    end
	  ...
	  begin
	    ...
	    if todos = 0 then
	      ...
	      then (write_org bugfile_ext prefix orgs2; (*edition du .new.org *)
		    ...
		    Bolt.Logger.log log Bolt.Level.INFO "")  
	      else
		...
		Org.sort annots;
 		if not (cleanorgs = annots) then
		  ...
		  begin
			...
			Bolt.Logger.log msggg Bolt.Level.INFO "";
		      end
		  end
	    else
	      if !Misc.debug then
...
let run_correl_job v1 v2 v3 cpucore diffalgo file =
  let pid = Unix.fork () in
	...
	(if !Misc.debug then
	    ...
	    Bolt.Logger.log logmsg4 Bolt.Level.TRACE ""
	);
...
let correl v1 v2 v3 configfile diffalgo filter =
  ...
  List.iter (fun (s, n) -> let logmsg=Printf.sprintf "%d %s" s n in Bolt.Logger.log logmsg Bolt.Level.DEBUG "") order;
  ...
  if error <> 0 then
    let logmsg3=Printf.sprintf "*** ERROR *** %d error(s) during the correlation." error in
      Bolt.Logger.log logmsg3 Bolt.Level.ERROR ""
