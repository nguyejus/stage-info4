...
let parse_cache cache_file =
  ...
  with _ -> (let logmsg=Printf.sprintf "File %s does not exit, run make preinit." cache_file in Bolt.Logger.log logmsg Bolt.Level.ERROR "");
...
let get_xmin db g atts =
  ...
  with _ ->
    ...
    Bolt.Logger.log logmsg Bolt.Level.DEBUG "";
    0.0
...
let get_correl_mode verbose pattern =
  try
	...
	Bolt.Logger.log logmsg2 Bolt.Level.TRACE "";
	Ast_config.Default
  with Not_found ->
	 ...
	 try
	   let d_list = get_pattern false atts pattern catts in
	     ...
	     List.iter (fun x ->
			  ...
			  Bolt.Logger.log logmsg3 Bolt.Level.TRACE ""
		       ))
	       catts;
	 with
	     Misconfiguration s ->
  ...
  Debug.profile_code "Config.show_config"
    (fun () ->
	 ...
	 (fun name (_,atts) ->
	   ...
	   Bolt.Logger.log logmsg Bolt.Level.DEBUG "");
	   List.iter (fun a -> (let logmsg=Printf.sprintf "%s" (show_attr a) in Bolt.Logger.log logmsg Bolt.Level.TRACE "")) atts
	 )
	 Setup.projects;
       Setup.DftTbl.iter
	 (fun name atts ->
  ...
  match subgraph with
	...
	let (crv_wc, crv_woc, colors) = List.fold_left
	  (fun (crv_wc, crv_woc, colors) curve ->
		 ...
		 Bolt.Logger.log logmsg Bolt.Level.ERROR "");
