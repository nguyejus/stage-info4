...
let gen_makefile v1 v2 prjs patts exps =
  let deps = List.map
    (fun (p, cmds) ->
       let prjdir = Config.get_prjdir p in
       if prjdir = "" then (let logmsg=Printf.sprintf "*** WARNING *** project dir is not set for %s" p in Bolt.Logger.log logmsg Bolt.Level.WARN "");
       ...
       Bolt.Logger.log logmsg2 Bolt.Level.INFO "";
  ...
  Bolt.Logger.log logmsg3 Bolt.Level.INFO "";
  ...
  Bolt.Logger.log logmsg4 Bolt.Level.INFO "";
