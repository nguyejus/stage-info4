...
and check_next verbose strict conf prefix depth vlist diffs correl (bugs:Ast_org.orgarray) (bug:Ast_org.bug) check_pos
  ...
  Debug.profile_code_silent "check_next"
    (fun () ->
      ...
      List.iter (fun bug -> (let logmsg=Printf.sprintf "%s" (Org.show_bug true bug) in Bolt.Logger.log logmsg Bolt.Level.TRACE "")) subbugs;
      Bolt.Logger.log "" Bolt.Level.TRACE "";
      if subbugs = [] then
	begin
	  Bolt.Logger.log "Automatic correlation OK" Bolt.Level.TRACE "";
  ...
  Debug.profile_code_silent "compute_org"
	...
	begin
	  ...
	  pariter cpucore (fun cmd ->
		...
		| Unix.WEXITED i -> (let logmsg=Printf.sprintf "*** FAILURE *** Code: %d %s" i cmd in Bolt.Logger.log logmsg Bolt.Level.ERROR ""); false
		| _ -> (let logmsg=Printf.sprintf "*** FAILURE *** %s" cmd in Bolt.Logger.log logmsg Bolt.Level.ERROR ""); false
	    in
	    if status then
