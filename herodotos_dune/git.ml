...
let get_tags scmpath expression =
  ...
  let tag_list =
    ...
    in rl ()
  in
  List.iter (fun tag -> (let logmsg=Printf.sprintf "Tag: %s" tag in Bolt.Logger.log logmsg Bolt.Level.DEBUG "")) tag_list;
  tag_list

let get_version_date path version deposit =
  let gitpath = path ^ "/" ^ deposit in
  try
    Bolt.Logger.log "Retrieving information about version %s from %s" Bolt.Level.INFO (version ^ gitpath);
