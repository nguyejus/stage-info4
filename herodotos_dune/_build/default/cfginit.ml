...
let gen_cvsignore_prj v1 (p, _) =
  ...
  let cvsignore = !Setup.resultsdir ^ prjdir^"/.cvsignore" in
    if not (Sys.file_exists cvsignore) then
      Bolt.Logger.log "Generating %s" Bolt.Level.INFO cvsignore;
...
let gen_makefile_patt verbose depch cmds =
  List.fold_left (fun datalist (data, verscmds) ->
    Bolt.Logger.log "make will generate data in %s" Bolt.Level.INFO data;
...
let gen_makefile v1 v2 prjs patts exps =
  let deps = List.map
    (fun (p, cmds) ->
       let prjdir = Config.get_prjdir p in
       if prjdir = "" then Bolt.Logger.log "*** WARNING *** project dir is not set for %s" Bolt.Level.WARN p;
       let depend = !Setup.resultsdir ^ prjdir ^"/.depend."^p in
       let depch = Misc.create_dir_and_open v1 depend in
       Bolt.Logger.log "Generating %s" Bolt.Level.INFO depend;
  ...
  Bolt.Logger.log "Generating %s" Bolt.Level.INFO pattdepend;
...
let check_prj vb p =
  ...
  List.fold_left (fun l v ->
    ...
    if not (Sys.file_exists prj) then
      ...
      (
	Bolt.Logger.log "%s - OK" Bolt.Level.TRACE prj;
	l
      )
  ) [] versions
...
let check_pat vb d =
  ...
  if not (Sys.file_exists cocci) then
    ...
    (
      Bolt.Logger.log "%s - OK" Bolt.Level.TRACE cocci;
      []
    )
...
let init_env v1 v2 v3 configfile cvs =
  ...
  if err = [] then
    ...
    (
      Bolt.Logger.log " *** KNOWN PROBLEMS ***" Bolt.Level.ERROR "";
      List.iter (fun msg -> Bolt.Logger.log msg Bolt.Level.ERROR "") err;
      Bolt.Logger.log " *** END OF REPORT ***" Bolt.Level.ERROR ""
    )
