...
let may_have_changed strict prefix vlist bfl bug =
  ...
  if next = {Ast_org.def = None} then Bolt.Logger.log "may_have_changed: no next computed" Bolt.Level.TRACE "";
...
let is_STATUS_as_next_in_correl status correl file ver pos =
  Bolt.Logger.log "is_STATUS_as_next_in_correl: start - %s/%s for status %s"
    Bolt.Level.TRACE (ver ^ file ^ (Org_helper.get_status status));
  let res =
    List.exists
  ...
  in
  Bolt.Logger.log "is_STATUS_as_next_in_correl: %s/%s %b" Bolt.Level.TRACE (ver ^ file ^ string_of_bool(res));
  res
...
let find_all_next strict prefix vlist (orgsarray:Ast_org.orgarray) correl bug =
  ...
  let (_, cb, ce) = pos in
  Bolt.Logger.log "find_all_next: checking for %s/%s%s - %s"
    Bolt.Level.TRACE (ver ^ file ^ (Org.get_string_pos pos) ^ (Org.clean_link_text prefix ver file pos t));
    (*
	   ...
	   file = file2 && vn = ver2
	     ...
	     *)
	     ...
	     && (if strict then
		   ...
		   Bolt.Logger.log "find_all_next: \"%s\" <-> \"%s\"" Bolt.Level.TRACE (new_t ^ new_tb);
		   new_t = new_tb
		 else true)
	     ...
	     *)
	     && not (List.exists
			 ...
			 Bolt.Logger.log "find_all_next: check %s/%s%s def:%s" Bolt.Level.TRACE (pver ^ pfile ^ (Org.get_string_pos ppos) ^ (show_opt next.Ast_org.def));
...
let exists_bug_for_correl vlist orgs cbug =
  ...
  Bolt.Logger.log "exists_bug_for_correl: check in %s from %s%s to %s%s"
    Bolt.Level.TRACE (cfile ^
    cver ^ (Org.get_string_pos cpos) ^
    cnver ^ (Org.get_string_pos cnpos));
  List.exists (fun bug ->
    let (_, _, _, file, ver, pos, _, _, _, next, _) = bug in
    Bolt.Logger.log "exists_bug_for_correl: %s def:%s" Bolt.Level.TRACE ((Org.show_bug true bug) ^ (show_opt next.Ast_org.def));
    cpos = pos
    && cs = Ast_org.SAME
      && not (is_auto bug)
...
let get_t_of_bug_for_next_correl strict prefix vlist orgs cbug =
  ...
  Bolt.Logger.log "get_t_of_bug_for_next_correl: check for %s/%s%s" Bolt.Level.TRACE (cver ^ cfile ^ (Org.get_string_pos cpos));
  ...
  Bolt.Logger.log "get_t_of_bug_for_next_correl: check \"%s\" <-> \"%s\"" Bolt.Level.TRACE (ct ^ new_t);
  new_t
...
let get_all_correl strict prefix vlist correl orgs bug =
  ...
  Bolt.Logger.log "get_all_correl: start - %s - %s" Bolt.Level.TRACE ((Org.show_bug true bug) ^ new_tb);
  List.find_all
    (fun cbug ->
      let (cs, cfile, cver, cpos, cnfile, cnver, cnpos, t) = cbug in
      cfile = file
	...
	&& cpos = pos
	  && (if strict then
		...
		let new_t = Org.clean_link_text prefix ver file pos t in
		Bolt.Logger.log "get_all_correl: %s \"%s\" <-> \"%s\"" Bolt.Level.TRACE ((Org.show_bug true bug) ^ new_t ^ new_nt);
		new_t = new_tb && new_t = new_nt
	      with Not_found ->
		Bolt.Logger.log "get_all_correl: Not_found raised while checking for %s/%s%s" Bolt.Level.WARN (ver ^ file ^ (Org.get_string_pos pos));
		false
	    else true)
    ) correl
...
let gen_todo ch strict vlist prefix orgs correl bug =
  ...
  *)
  Bolt.Logger.log "gen_todo called for %s" Bolt.Level.TRACE (Org.show_bug true bug);
...
let correlate verbose strict prefix vlist correlfile prefix unclean_correl orgsarray orgs =
  ...
  let todo = List.map (fun bug ->
    Bolt.Logger.log "correlate: Start mapping..." Bolt.Level.TRACE "";
    try
      (*
	...
	*)
	...
	let todo = List.map
	  (fun (s, file, ver, pos, nfile, nver, npos, t) ->
	    Bolt.Logger.log "correlate: Start nested map" Bolt.Level.TRACE "";
	    let nbug =
    ...
    with e ->
      Bolt.Logger.log "EXN %s" Bolt.Level.FATAL (Printexc.to_string e);
	...
	if exists_bug_for_correl vlist orgsarray cbug then
	  ...
	  *)
	  ...
	  Bolt.Logger.log "*** INFO *** Drop an old SAME entry of a now non-existing bug: %s" Bolt.Level.INFO (Org.show_bug true bug)
    ) correl;
