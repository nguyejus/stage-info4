...
let get_diff_job v prefix difffile cmd =
  Bolt.Logger.log "Running: %s" Bolt.Level.TRACE cmd;
  match
    Unix.system cmd
  with
      Unix.WEXITED 0 -> ()
...
let run_get_diff_job v prefix difffile cmd =
  let pid = Unix.fork () in
	...
	Bolt.Logger.log "New child %d for %s" Bolt.Level.TRACE (string_of_int(pid) ^ cmd);
	let ret = get_diff_nofail v prefix difffile cmd in
	Bolt.Logger.log "Job done for child %d" Bolt.Level.TRACE ("" ^ string_of_int(pid));
...
let gen_cmd_basic v prefix pair difffile =
  Bolt.Logger.log "*** CHECK CACHE *** %s" Bolt.Level.INFO (get_difffile difffile);
  Parmap.parfold (fun file_pair (outlist, cmdlist) ->
    ...
    if not (Sys.file_exists outfile) then
      (Bolt.Logger.log "Checking (%d) %s - Keep" Bolt.Level.TRACE (string_of_int(Unix.getpid ()) ^ outfile);
       (outfile::outlist,(outfile, cmd)::cmdlist))
    else
      (Bolt.Logger.log "Checking (%d) %s - Skip" Bolt.Level.TRACE (string_of_int(Unix.getpid ()) ^ outfile);
...
let mkdir_cache_basic file =
  if not (Sys.file_exists file) then
    (Unix.mkdir file 0o770;
     Bolt.Logger.log "*** CREATING DIRECTORY *** %s" Bolt.Level.INFO file)
...
let get_diff v cpucore resultsdir pdir prefix vlist (orgs: Ast_org.orgarray) orgfile difffile : Ast_diff.diffs =
  ...
  in
  if error <> 0 then
    Bolt.Logger.log "*** ERROR *** %d error(s) during the diff." Bolt.Level.ERROR ("" ^ string_of_int(error));
  match difffile with
      GNUDiff _ ->
	List.flatten (
	  Parmap.parmap (fun x ->
	    ...
	    with e -> Bolt.Logger.log "Error parsing %s" Bolt.Level.ERROR x; raise e
	  ) (Parmap.L outfiles)
	)
    | Gumtree _ -> 
      List.flatten (
...
let show_diff verbose vlist ast =
  Bolt.Logger.log "SHOW DIFF" Bolt.Level.TRACE "";
  List.iter (fun ((ver, file), difftype) ->
    Bolt.Logger.log "%s from %s to %s" Bolt.Level.TRACE (file ^ ver ^ (Misc.get_next_version vlist ver));
    match difftype with
	Ast_diff.GNUDiff hunks -> Bolt.Logger.log "%s" Bolt.Level.TRACE (show_gnudiff hunks)
