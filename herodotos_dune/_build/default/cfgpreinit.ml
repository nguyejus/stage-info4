...
let build_updated_cache cache_projects withsizes =
  Setup.PrjTbl.fold 
    (fun prj _ cache ->
      Bolt.Logger.log "Processing %s for cache" Bolt.Level.INFO prj;
      (* Check for a regular expression *)
      let re = Config.get_versionsRE prj in
      try
	let cacheddata = List.assoc prj cache_projects in
	Bolt.Logger.log "Data found in the cache for %s" Bolt.Level.INFO prj;
	if re <> "" then
      ...
      with Not_found ->
	Bolt.Logger.log "No data in cache for %s" Bolt.Level.INFO prj;
	let versinfos =
	  try
	    List.map (fun (name, days, date, size) -> (name, date, size))
	      (Array.to_list (snd (Config.get_versinfos prj)))
	  with _ ->
	    Bolt.Logger.log "No version information manually provided." Bolt.Level.DEBUG "";
	    []
	in
	if re <> "" then
	  ...
	  if versinfos = [] then
	    begin
	      Bolt.Logger.log "No data source provided." Bolt.Level.FATAL "";
	      failwith "No data source provided."
	    end
	  else
	    begin
	      Bolt.Logger.log "Use only user provided data." Bolt.Level.INFO "";
	      (prj, versinfos)::cache
	    end
    )
    Setup.projects []
