...
let clean_file exist editfile =
  if Sys.file_exists editfile then
    begin
      if exist then Bolt.Logger.log "*** INFO *** %s is thus removed." Bolt.Level.INFO editfile;
      Sys.remove editfile
    end
  ...
  let editfile   = bugfile ^ Global.editext in
	...
	if (not ((Config.get_correl_mode v1 patt) = Ast_config.Nocorrel)) then
	  begin
	    ...
	    Bolt.Logger.log msg Bolt.Level.INFO "";
	    if !Misc.debug then
	    ...
	    if todos = 0 then
	      ...
	      then (write_org bugfile_ext prefix orgs2; (*edition du .new.org *)
		    Bolt.Logger.log "*** NEW FILE TO EDIT *** %s" Bolt.Level.INFO bugfile_ext)  
	      else
		...
		Org.sort annots;
 		if not (cleanorgs = annots) then
		  Bolt.Logger.log "*** NEW FILE TO EDIT *** %s" Bolt.Level.INFO editfile
		else
		  begin
		    Bolt.Logger.log "*** INFO *** %s is up to date." Bolt.Level.INFO bugfile_ext;
		    if not !Misc.debug
			...
			Bolt.Logger.log "*** INFO *** %s contains a sorted version." Bolt.Level.INFO editfile;
		      end
		  end
	    else
	      if !Misc.debug then
	  ...
	  Bolt.Logger.log "*** NO CORREL *** %s" Bolt.Level.INFO orgfile
    else
      if Config.get_format patt = Ast_config.Org then
	Bolt.Logger.log "*** SKIP (NOT FOUND) *** %s" Bolt.Level.WARN orgfile
...
let correl_patt_prj_nofail v1 v2 v3 cpucore diffalgo file =
  try
    ...
    with Config.Warning msg ->
      Bolt.Logger.log "*** WARNING *** %s" Bolt.Level.WARN msg;
      1
  with
      Misc.Strip msg ->
	Bolt.Logger.log "*** FATAL *** Precessing %s" Bolt.Level.FATAL file;
	Bolt.Logger.log "*** FATAL *** %s" Bolt.Level.FATAL msg;
	Debug.trace (Printexc.get_backtrace ());
	1
    | e ->
      Bolt.Logger.log "*** FATAL *** %s" Bolt.Level.FATAL (Printexc.to_string e);
      Debug.trace (Printexc.get_backtrace ());
      1
...
let run_correl_job v1 v2 v3 cpucore diffalgo file =
  let pid = Unix.fork () in
	...
	(if !Misc.debug then
	    ...
	    Bolt.Logger.log "New child %d on %s" Bolt.Level.TRACE (string_of_int(pid) ^ file)
	 else
	    Bolt.Logger.log "New child on %s" Bolt.Level.TRACE file);
	let ret = correl_patt_prj_nofail v1 v2 v3 cpucore diffalgo file in
	(if !Misc.debug then
	    Bolt.Logger.log "Job done for child %d - exit %d" Bolt.Level.TRACE (string_of_int(pid) ^ string_of_int(ret))
	 else
	    Bolt.Logger.log "Job done for child - exit %d" Bolt.Level.TRACE ("" ^ string_of_int(ret))
	);
...
let dispatch_correl_job v1 v2 v3 cpucore diffalgo (perr, pidlist) file :int*int list =
  let (error, newlist) =
	...
	| _ -> true
      in
      (if !Misc.debug then
	  Bolt.Logger.log "Master: Job done for child %d - error status %b" Bolt.Level.TRACE (string_of_int(death) ^ string_of_bool(child_err))
       else
	  Bolt.Logger.log "Master: Job done for child - error status %b" Bolt.Level.TRACE ("" ^ string_of_bool(child_err))
      );
...
let correl v1 v2 v3 configfile diffalgo filter =
  ...
  List.iter (fun (s, n) -> Bolt.Logger.log "%d %s" Bolt.Level.DEBUG (string_of_int(s) ^ n)) order;
  ...
  in
    if error <> 0 then
      Bolt.Logger.log ("*** ERROR *** %d error(s) during the correlation.") Bolt.Level.ERROR ("" ^ string_of_int(error))
