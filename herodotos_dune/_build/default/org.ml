...
let show_buglist verbose bugs =
  count := 0;
  List.iter (fun bug ->
    Bolt.Logger.log "#%03d %s" Bolt.Level.TRACE (string_of_int(!count) ^ (show_bug verbose bug));
    count := !count + 1;
  ) bugs
...
let show_org verbose prefix (orgs: (string*Ast_org.bugs  list) list) =
  ...
  Bolt.Logger.log "Prefix used: %s" Bolt.Level.TRACE prefix;
  List.iter (fun (file, bugslist) ->
	...
	(List.map (show_bug verbose) bugs)
      in
      Bolt.Logger.log "#%03d in vers. %s" Bolt.Level.TRACE (string_of_int(!count) ^ vers);
      count := !count + 1
    ) bugslist;
...
let compute_correlation prefix depth correl =
  ...
  try
	...
	(s, file, ver, pos, nfile, nver, npos, new_t)::correllist
      with Failure msg ->
	Bolt.Logger.log "compute_correlation: failure with %s" Bolt.Level.FATAL msg;
	correllist
    ) [] list
  with Misc.Strip msg ->
    Bolt.Logger.log "compute_correlation: failure with %s" Bolt.Level.FATAL msg;
