...
let rec read_diff prefix file in_ch =
  ...
  in
	...
	begin
	  Bolt.Logger.log "Error: Desynchronized? no more diff found in %s at line %s" Bolt.Level.ERROR ~file:file (Int64.to_string !line);
	  Bolt.Logger.log "Expects a \"---\" line. Got \"%s\"" Bolt.Level.ERROR line_rm0;
	  Bolt.Logger.log "Looking for filename in \"%s\"" Bolt.Level.ERROR line_rm;
	  match lookfor_minusline in_ch with
	      None ->
		Bolt.Logger.log "Unable to re-synchronize" Bolt.Level.ERROR ""; []
	    | Some l ->
		Bolt.Logger.log "Re-synchronize on %s" Bolt.Level.INFO l;
		match lookfor_minusline in_ch with
		    None   -> []
		  | Some l ->
...
let parse_diff v prefix file : Ast_diff.diffs =
  try    
    ...
    if Sys.file_exists file then
      begin
	Bolt.Logger.log "Parsing GNU Diff: %s" Bolt.Level.TRACE file;
	...
	with
	  ...
	  | (Unexpected msg) ->
	    Bolt.Logger.log "Unexpected token: %s" Bolt.Level.ERROR msg;
	    close_in in_ch;
	    raise (Unexpected msg)
	  | End_of_file ->
	    Bolt.Logger.log "*** DEBUG *** %s is empty !" Bolt.Level.DEBUG file;
	    close_in in_ch;
	  []
      end
    else
