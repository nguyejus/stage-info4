...
let parse_cache cache_file =
  ...
  with _ -> Bolt.Logger.log "File %s does not exit, run make preinit." Bolt.Level.ERROR cache_file;
...
let show_attr attr : string =
  match attr with
    ...
    | Ast_config.Version (_, vs) ->
      (try
	 Bolt.Logger.log "version = {" Bolt.Level.TRACE "";
	 let m = get_vers_min vs in
	 List.iter (fun v -> Bolt.Logger.log "%s" Bolt.Level.TRACE (show_version m v)) vs;
	 "}"
...
let get_xmin db g atts =
  try
	...
	Ast_config.XMin v -> v
      | _ -> raise Unrecoverable
  with _ ->
    Bolt.Logger.log "XMin of %s is not defined. Assuming 0." Bolt.Level.DEBUG g;
    0.0
...
let get_correl_mode verbose pattern =
  try
	...
	with
	    Ast_config.Correl s ->
	      Bolt.Logger.log "Pattern %s has correlation mode: %s" Bolt.Level.TRACE (pattern ^ s);
	      (match s with
	  ...
	  | _ -> raise Unrecoverable
      with _ ->
	Bolt.Logger.log "Pattern %s has no correlation mode set: default assumed" Bolt.Level.TRACE pattern;
	Ast_config.Default
  with Not_found ->
    ...
    (fun () ->
	 ...
	 try
	   let d_list = get_pattern false atts pattern catts in
	     if d_list = [] then
	       Bolt.Logger.log "No need to compute data on bugs for this curve." Bolt.Level.TRACE ""
	     else
	       begin
		 let p = get_project (catts@atts) project in
		   List.iter (fun d ->
		     Bolt.Logger.log "Curve with p:%s and d:%s " Bolt.Level.TRACE (p ^ d)
			     ) d_list;
		   List.iter (fun d ->
				let data = locate_data p d Global.origext in
				Bolt.Logger.log "Reading data from: %s" Bolt.Level.TRACE data
			     ) d_list
	       end;
	     List.iter (fun x ->
			  Bolt.Logger.log "\t%s" Bolt.Level.TRACE (show_attr x)
		       )
	       catts;
	 with
	     Misconfiguration s ->
...
let show_global () =
  Bolt.Logger.log "Prefix = %s" Bolt.Level.DEBUG !Setup.prefix;
  Bolt.Logger.log "Cocci files in %s" Bolt.Level.DEBUG !Setup.smatchdir;
  Bolt.Logger.log "Project source code in %s" Bolt.Level.DEBUG !Setup.projectsdir;
  Bolt.Logger.log "Analysis results in %s" Bolt.Level.DEBUG !Setup.resultsdir;
  Bolt.Logger.log "FindCmd = \"%s\"" Bolt.Level.DEBUG !Setup.findcmd;
  Bolt.Logger.log "Flags = \"%s\"" Bolt.Level.DEBUG !Setup.spflags;
  match !Setup.cpucore with
      None ->
	Bolt.Logger.log "CPU core = <undefined: assuming %d>" Bolt.Level.DEBUG ("" ^ string_of_int(Misc.get_number_of_cores ()))
    | Some cpucore ->
	Bolt.Logger.log "CPU core = %d" Bolt.Level.DEBUG ("" ^ string_of_int(cpucore))
...
let show_config () =
  Debug.profile_code "Config.show_config"
    (fun () ->
       ...
       Setup.PrjTbl.iter
	 (fun name (_,atts) ->
	   Bolt.Logger.log "Project %s" Bolt.Level.DEBUG name;
	   List.iter (fun a -> Bolt.Logger.log "%s" Bolt.Level.TRACE (show_attr a)) atts
	 )
	 Setup.projects;
       Setup.DftTbl.iter
	 (fun name atts ->
	   Bolt.Logger.log "Pattern %s" Bolt.Level.DEBUG name;
	   List.iter (fun a -> Bolt.Logger.log "%s" Bolt.Level.TRACE (show_attr a)) atts
	 )
	 Setup.smatchs;
       Setup.GphTbl.iter
	 (fun name (atts, subgraph) ->
	   Bolt.Logger.log "Graph %s" Bolt.Level.DEBUG name;
	   List.iter (fun a -> Bolt.Logger.log "%s" Bolt.Level.TRACE (show_attr a)) atts;
	   match subgraph with
  ...
  match subgraph with
	...
	let (crv_wc, crv_woc, colors) = List.fold_left
	  (fun (crv_wc, crv_woc, colors) curve ->
		 ...
		 Bolt.Logger.log "%s Automatically picking one..." Bolt.Level.ERROR msg;
