...
let retrieve_exists verbose prjname mode bugsetfile vlist filelist =
  ...
  let fromfile =
	...
	try
	  ...
	  Bolt.Logger.log "Cache file %s was used." Bolt.Level.DEBUG existfile;
	  ret
	with
	    Sys_error msg -> prerr_endline msg; None
	  | Exists.BadFormat -> None
    else
      (Bolt.Logger.log "Cache file %s is obsolete or does not exist." Bolt.Level.DEBUG existfile;
       None)
  in
    match fromfile with
	Some ast -> ast
      | None ->
	  Bolt.Logger.log "*** INFO *** Compute %s from repository %s." Bolt.Level.INFO (existfile ^ (!Setup.projectsdir ^ prjdir));
...
let compute_pbugs verbose prjname allbugs =
  ...
  with
      Config.Warning msg -> Bolt.Logger.log msg Bolt.Level.WARN ""; None
    | Error msg -> Bolt.Logger.log msg Bolt.Level.ERROR ""; None
...
let compute_bugs verbose bugfile =
  try
    ...
    *)
	...
	then
	  (Bolt.Logger.log "Analyzing %s in project %s with subproject depth %d." Bolt.Level.TRACE (bugfile ^ p ^ string_of_int(depth));
	  ...
	  begin
		...
		let orgs  = Org.format_orgs prefix depth (Org.parse_org false orgfile) in
	      ...
	      (Bolt.Logger.log "*** ERROR *** No file %s" Bolt.Level.ERROR bugfile;
	       []
	      )
	  end
    in
	...
	in
	if count <> 0 then
	  Bolt.Logger.log "*** WARNING *** Dropping %d bug report(s) of %s (No interesting version) !" Bolt.Level.WARN (string_of_int(count) ^ bugfile);
...
let compute_graphs verbose graph =
  Debug.profile_code "Cfghelper.compute_graphs"
    (fun () ->
       ...
       List.iter (fun d -> Bolt.Logger.log d Bolt.Level.TRACE "") bugfiles;
       Bolt.Logger.log "==========================" Bolt.Level.TRACE "";
       let pbugs =
