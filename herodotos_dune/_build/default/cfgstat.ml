    ...
    if (not ((Config.get_correl_mode v1 patt) = Ast_config.Nocorrel)) then
      try
		...
		begin
		  ...
		  let _ = if strict then Bolt.Logger.log "*** INFO *** Strict correlation used for the pattern %s" Bolt.Level.INFO patt in
		...
		Bolt.Logger.log "%s R:% 6d %s / %s -> %s" Bolt.Level.INFO (duple ^ string_of_int(reports) ^ correl ^ auto ^ fp)
	      else
		if cinfo then
		  Bolt.Logger.log "%s %s" Bolt.Level.INFO (duple ^ correl)
		else
		  Bolt.Logger.log "%s %s" Bolt.Level.INFO (duple ^ fp)
      with err ->
	    Bolt.Logger.log "%s Skipping..." Bolt.Level.ERROR duple;
	    if !Misc.debug then raise err;
	else
	  Bolt.Logger.log "%s (no correlation requested) Skipping..." Bolt.Level.ERROR duple
...
let stats v1 v2 v3 configfile graph statmode =
  ...
  Bolt.Logger.log "Running in stat mode: correl stat %b / fp stat %b" Bolt.Level.INFO (string_of_bool(correl) ^ string_of_bool(fp));
