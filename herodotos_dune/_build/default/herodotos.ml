...
let options = [
  ...
  "--profile", Arg.Unit (fun () -> Bolt.Logger.log "*** PROFILING ENABLED ***" Bolt.Level.TRACE "";
  ...
  match !mode with
	...
	begin
	  match running_mode with
		...
		Bolt.Logger.log "CPU: %d" Bolt.Level.DEBUG ("" ^ string_of_int(Parmap.get_default_ncores ()))
	    | Help | Longhelp ->
		...
		begin
		  ...
		  match running_mode with
			...
			(fun () -> 
			  ...
			  then (
			    if ((String.length !prefix) <> 0) then (
			      ...
			      if ast = [] then
				Bolt.Logger.log "Empty Org file: %s" Bolt.Level.WARN !orgfile
			      else
				begin
				  Bolt.Logger.log "Checking %d elements..." Bolt.Level.INFO ("" ^ string_of_int(List.length ast));
				  let formatted =
				    try
				      Org.format_orgs !prefix 1 ast
				    with Misc.Strip msg ->
				      Bolt.Logger.log "Error: %s" Bolt.Level.FATAL msg;
				      failwith msg
				  in
				  Bolt.Logger.log "Converting %d elements..." Bolt.Level.INFO ("" ^ string_of_int(List.length formatted));
				  if formatted = [] then
				    Bolt.Logger.log "Conversion failed!" Bolt.Level.ERROR ""
				  else
				    let filtered =
				      if !extract = "" then
					(Bolt.Logger.log "No extraction to perform" Bolt.Level.INFO "";
					 formatted)
				      else
					(Bolt.Logger.log "Extracting version tagged '%s'" Bolt.Level.INFO !extract;
				    ...
				    with _ -> Bolt.Logger.log "Fail to write to %s" Bolt.Level.ERROR !outfile;
				end
			    ) else (
			      Bolt.Logger.log "*** ERROR *** Prefix not set" Bolt.Level.ERROR "";
			      failwith "No prefix set"
			    )
			  ) else (
			    Bolt.Logger.log "*** ERROR *** Org file not set" Bolt.Level.ERROR "";
			...
			(fun () ->
			  Bolt.Logger.log "Herodotos version %s" Bolt.Level.INFO Global.version;
			  Bolt.Logger.log "Processing %s" Bolt.Level.INFO !configfile;
			...
			( fun () ->
			  ...
			  if ((String.length !orgfile) <> 0) then
			    begin
			      if ((String.length !prefix) = 0) then Bolt.Logger.log "*** WARNING *** Prefix not set" Bolt.Level.WARN "";
			      Bolt.Logger.log "Parsing...\n" Bolt.Level.INFO "";
			      let ast = Org.parse_org false !orgfile in
			      if ast = [] then
				Bolt.Logger.log "Empty Org file" Bolt.Level.WARN ""
			      else
				begin
				  Bolt.Logger.log "Checking... (%d elements)" Bolt.Level.INFO ("" ^ string_of_int(List.length ast));
				  if !Misc.debug then
				  ...
				  in
				  Bolt.Logger.log "Converting... (%d elements)" Bolt.Level.INFO ("" ^ string_of_int(List.length formatted));
 				  if msg <> "" then Bolt.Logger.log "%s" Bolt.Level.ERROR msg;
				  if formatted = [] then
				    Bolt.Logger.log "Failed!" Bolt.Level.FATAL ""
				  else
				    (if !sql then
...
let _ =
  ...
  Bolt.Logger.log "*** START ***" Bolt.Level.TRACE "";
  Array.iteri (fun i opt -> Bolt.Logger.log "Option %d: %s" Bolt.Level.TRACE (string_of_int(i) ^ opt)) Sys.argv;
  let aligned = Arg.align options in
    (try
      Arg.parse_argv Sys.argv aligned anon_fun usage_msg;
    with Arg.Bad msg ->
      (Bolt.Logger.log msg Bolt.Level.FATAL ""; exit 0));
  (try main aligned
   with e ->
     Bolt.Logger.log "Exception: %s" Bolt.Level.FATAL (Printexc.to_string e);
     Debug.trace (Printexc.get_backtrace ());
     exit 1);
  if !Debug.profile <> Debug.PNONE then
    Debug.trace (Debug.profile_diagnostic ());
  Bolt.Logger.log "*** END ***" Bolt.Level.TRACE ""
