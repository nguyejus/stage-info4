...
module MenhirBasics = struct
  ...
  type token = 
    ...
    | TSC
    | TInt of (
# 8 "exists/exist_parser.mly"
      (int)
# 17 "exists/exist_parser.ml"
  )
    | TId of (
# 7 "exists/exist_parser.mly"
      (string)
# 22 "exists/exist_parser.ml"
  )
    | TFalse
    | EOL
    | EOF
...
# 1 "exists/exist_parser.mly"
  


# 36 "exists/exist_parser.ml"
...
and ('s, 'r) _menhir_cell1_separated_nonempty_list_sep_vid_ = 
  | MenhirCell1_separated_nonempty_list_sep_vid_ of 's * ('s, 'r) _menhir_state * (string list)

and ('s, 'r) _menhir_cell1_state_info = 
  | MenhirCell1_state_info of 's * ('s, 'r) _menhir_state * (Ast_exist.state)

and ('s, 'r) _menhir_cell1_vid = 
  | MenhirCell1_vid of 's * ('s, 'r) _menhir_state * (string)
...
let _menhir_action_02 =
  fun es ->
    (
# 39 "exists/exist_parser.mly"
                                     ( es     )
# 128 "exists/exist_parser.ml"
     : (Ast_exist.entry list))

let _menhir_action_03 =
  fun id states ->
    let _2 = 
# 60 "exists/exist_parser.mly"
     ()
# 136 "exists/exist_parser.ml"
     in
    (
# 43 "exists/exist_parser.mly"
    ( (id,
       List.filter
	 (fun s ->
	   match s with
...
# 60 "exists/exist_parser.mly"
     ()
# 189 "exists/exist_parser.ml"
     in
    (
# 21 "exists/exist_parser.mly"
    ( (
      List.filter
	(fun vname ->
	if vname = "" then
...
# 60 "exists/exist_parser.mly"
     ()
# 221 "exists/exist_parser.ml"
     in
    (
# 240 "<standard.mly>"
    ( x :: xs )
# 226 "exists/exist_parser.ml"
     : (Ast_exist.state list))
...
# 60 "exists/exist_parser.mly"
     ()
# 242 "exists/exist_parser.ml"
     in
    (
# 240 "<standard.mly>"
    ( x :: xs )
# 247 "exists/exist_parser.ml"
     : (string list))

let _menhir_action_13 =
  fun () ->
    (
# 54 "exists/exist_parser.mly"
                                     ( Ast_exist.Empty    )
# 255 "exists/exist_parser.ml"
     : (Ast_exist.state))

let _menhir_action_14 =
  fun () ->
    (
# 55 "exists/exist_parser.mly"
                                     ( Ast_exist.True     )
# 263 "exists/exist_parser.ml"
     : (Ast_exist.state))

let _menhir_action_15 =
  fun () ->
    (
# 56 "exists/exist_parser.mly"
                                     ( Ast_exist.False    )
# 271 "exists/exist_parser.ml"
     : (Ast_exist.state))

let _menhir_action_16 =
  fun i ->
    (
# 57 "exists/exist_parser.mly"
                                     ( Ast_exist.Size (i) )
# 279 "exists/exist_parser.ml"
     : (Ast_exist.state))

let _menhir_action_17 =
  fun id ->
    (
# 35 "exists/exist_parser.mly"
          (id)
# 287 "exists/exist_parser.ml"
     : (string))

let _menhir_action_18 =
  fun () ->
    (
# 36 "exists/exist_parser.mly"
          ("")
# 295 "exists/exist_parser.ml"
     : (string))
